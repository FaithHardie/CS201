...
Answer: Histograms are normally used for continuous data and bar charts are used for categorical data.
...
```{r}
crimedata$agency_code <- as.factor(crimedata$agency_code)
crimedata$agency_jurisdiction <- as.factor(crimedata$agency_jurisdiction)
crimedata$report_year<- as.factor(crimedata$report_year)
```

#### C: Variation and Co variation

```{r}
ggplot(crimedata, aes(x = homicides_percapita)) +
  geom_histogram(binwidth = 15)
max(crimedata$homicides_percapita) +
  coord_cartesian(ylim = c(0, 350), xlim= c(0, 2000))
summary(crimedata$homicides_percapita)
```
